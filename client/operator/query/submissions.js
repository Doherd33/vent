...
export function openSubsDrawer() {
  // Reset to first tab
...
export function removeVentFile(i) {
...
function renderVentPreviews() {
  ...
  el.innerHTML = _ventFiles.map((f, i) => {
    ...
    return `<div class="vp-prev-item">
      ${inner}
      <button class="vp-prev-remove" data-action="removeVentFile" data-index="${i}" title="Remove">\u00d7</button>
    </div>`;
  }).join('');
}

export function toggleVentPanel() {
  document.getElementById('ventPanel').classList.contains('show') ? closeVentPanel() : openVentPanel();
}
export function openVentPanel() {
...
export function resetVentPanel() {
...
export function toggleVpRecording() {
...
async function startVpRecording() {
  ...
  if (!window._ventRecAudioCtx) window._ventRecAudioCtx = new (window.AudioContext || window.webkitAudioContext)();
  const audioCtx = window._ventRecAudioCtx;
