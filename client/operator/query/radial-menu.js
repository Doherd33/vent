...
export function registerRadialActions(map) {
  Object.assign(actionMap, map);
}

// ── Close-all callback for showVentHub ──
let _closeAllFn = null;

export function setCloseAllFn(fn) {
  _closeAllFn = fn;
}
...
export function applyMenuConfig(config){
...
export function dismissVentHub(){
  dismissHub();
  // Restore saved FAB position after hub dismisses
  setTimeout(restorePosition, 600);
}

export function showVentHub(){
  if(!hub) return;
  if(_closeAllFn) _closeAllFn();
  hubActive = true;
