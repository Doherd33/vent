...
import { _chatHistory, _currentSessionId, saveCurrentSession, fillAndSend, startNewChat } from './chat-history.js';
import { _camImageData, runVisualQuery } from './gdp.js';
      ...
      thinkEl.outerHTML = `<div class="answer-card"><div class="answer-body">
        <div class="error-card">
          ...
          <div class="error-card-actions">
            <button class="retry-btn" data-action="retryQuery" data-query="${retryQ}">Retry</button>
            <button class="retry-btn" style="border-color:var(--border);color:var(--mid)" data-action="startNewChat">New chat</button>
          </div>
        </div>
      </div></div>`;
    }
...
export function buildAnswerHtml(d) {
  const catMap = {
      ...
      d.sources.map((s, i) => {
        ...
        <div>
          <div class="source-item-row">
            ...
            <button class="src-expand-btn" data-action="toggleSrc" data-doc-id="${escHtml(s.code)}" data-section="${escHtml(s.section)}" data-uid="${uid}">
  ...
  if (d.followUps && d.followUps.length) {
    followHtml = `<div class="follow-ups">
      ...
      <div class="follow-up-chips">${d.followUps.map((f, i) =>
        `<div class="follow-up-chip" style="animation-delay:${400 + i * 120}ms" data-action="fillAndSend" data-query="${escHtml(f).replace(/'/g, "&#39;")}">${escHtml(f)}</div>`
      ).join('')}</div>
    </div>`;
  }
  ...
  const actionsHtml = `<div class="answer-actions">
    <button class="answer-act-btn" data-action="copyAnswer" data-card-id="${cardId}" title="Copy answer">
      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
      <span>Copy</span>
    </button>
    <button class="answer-act-btn" data-action="exportAnswerToDoc" data-card-id="${cardId}" title="Export to Doc Builder">
      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>
      <span>Export</span>
    </button>
    <div class="answer-actions-sep"></div>
    <button class="bookmark-btn${isBookmarked ? ' active' : ''}" id="bm-${cardId}" data-action="toggleBookmark" data-card-id="${cardId}" title="Pin answer">
    ...
    <button class="feedback-btn" id="fb-up-${cardId}" data-action="feedbackUp" data-card-id="${cardId}" title="Helpful">
    ...
    <button class="feedback-btn" id="fb-down-${cardId}" data-action="feedbackDown" data-card-id="${cardId}" title="Not helpful">
...
export async function toggleSrc(btn, docId, section, uid) {
...
// ── MARKDOWN RENDERER ──
export function renderSopMarkdown(md) {
  let out = md;
  ...
  // Inline formatting on remaining text (protect existing HTML blocks)
  const htmlBlocks = [];
  out = out.replace(/<(table|pre|blockquote|h[1-5])[\s\S]*?<\/\1>/gi, m => {
    htmlBlocks.push(m);
    return '\x00HTML' + (htmlBlocks.length - 1) + '\x00';
  });
  out = renderInline(out);
  out = out.replace(/\x00HTML(\d+)\x00/g, (_, i) => htmlBlocks[i]);
...
// ── COPY ANSWER ──────────────────────────────────────
export function copyAnswer(cardId) {
...
export function exportAnswerToDoc(cardId) {
  const d = window._answerData && window._answerData[cardId];
  if (!d) return;
...
// ── ANSWER FEEDBACK ──────────────────────────────────
export function feedbackAnswer(cardId, type) {
...
export function renderBookmarks() {
  ...
  list.innerHTML = bm.map(b => `
    <div class="bookmark-item" data-action="scrollToAnswer" data-id="${b.id}">
      <span class="bookmark-item-icon">${escHtml(b.icon)}</span>
      <span class="bookmark-item-text">${escHtml(b.summary)}</span>
      <button class="bookmark-item-remove" data-action="removeBookmark" data-id="${b.id}" title="Unpin">
...
export function scrollToAnswer(id) {
...
export function toggleBookmarksPanel() {
  const list = document.getElementById('bookmarksList');
