...
export function toggleDiscoverPanel() {
...
export async function discoverSops() {
  ...
  try {
    ...
    if (data.results && data.results.length) {
      ...
      data.results.forEach(r => {
        msgHtml += `<button class="discover-sop-link" data-action="openSopDoc" data-doc-id="${escHtml(r.doc_id)}">${escHtml(r.doc_id)}</button> `;
      });
      msgHtml += '</div>';
    }
...
export function closeSopSidebar() {
...
export async function searchSops() {
  ...
  try {
    ...
    results.innerHTML = data.map(row =>
      `<div class="sop-result-item" data-action="openSopDoc" data-doc-id="${escHtml(row.doc_id)}">
...
document.addEventListener('mousemove', function(e) {
  ...
  _svTx = e.clientX - _svDragX;
  _svTy = e.clientY - _svDragY;
  var viewer = document.getElementById('sopViewer');
  if (viewer) viewer.style.transform = 'translate3d(' + _svTx + 'px,' + _svTy + 'px,0)';
});
document.addEventListener('mouseup', function() {
  ...
  if (viewer) {
    ...
    viewer.style.left = (viewer.offsetLeft + _svTx) + 'px';
    viewer.style.top = (viewer.offsetTop + _svTy) + 'px';
...
export async function openSopDoc(docId) {
...
function renderSvTabs() {
  ...
  tabBar.innerHTML =
  `<button class="sv-add-btn${_svShowSearch ? ' active' : ''}" data-action="showSvSearch" title="Find another SOP">
  ...
  _svTabs.map(t =>
    `<div class="sv-tab${!_svShowSearch && t.id === _svActiveTab ? ' active' : ''}" data-action="switchSvTab" data-tab-id="${t.id}">
      <span class="sv-tab-id">${escHtml(t.docId)}</span>
      <button class="sv-tab-close" data-action="closeSvTab" data-tab-id="${t.id}">&times;</button>
    </div>`
  ).join('') +
  `<div class="sv-window-controls">
    <button class="sv-maximize-btn" data-action="toggleSvMaximize" title="${_svMaximized ? 'Restore' : 'Maximize'}">
      ${_svMaximized
    ...
    <button class="sv-close-btn" data-action="closeSopViewer" title="Close">
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
    </button>
  </div>`;
}

export function showSvSearch() {
  _svShowSearch = true;
  renderSvTabs();
  const body = document.getElementById('svBody');
  body.innerHTML = `
    <div class="sv-search-view">
      <div class="sv-search-header">
        ...
        <div class="sv-search-row">
          ...
          <button class="sv-search-go" id="svSearchBtn" data-action="runSvSearch">Search</button>
        </div>
      </div>
...
export async function runSvSearch() {
  const inp = document.getElementById('svSearchInput');
  ...
  try {
    ...
    results.innerHTML = data.map(row => {
      ...
      return `<div class="sv-search-item" data-action="openSvSearchResult" data-doc-id="${escHtml(row.doc_id)}">
...
export function switchSvTab(tabId) {
  _svShowSearch = false;
...
export function closeSvTab(tabId) {
...
function renderSvContent() {
  ...
  const emptyChatHtml = `<div class="sv-chat-empty">
    ...
    <div class="sv-suggestions">
      ${suggestions.map(s => `<div class="sv-sug" data-action="fillSvChat" data-question="${escHtml(s)}">${escHtml(s)}</div>`).join('')}
    </div>
  </div>`;
    ...
    <div class="sv-chat-pane">
      ...
      <div class="sv-chat-footer">
        <div class="sv-chat-input-row">
          ...
          <button class="sv-chat-send" id="svChatBtn" data-action="askSvDoc" title="Ask">
...
export function backToSopSearch() {
...
export function openTodoSidebar() {
  closeSopSidebar();
  loadTodos();
...
export function closeTodoSidebar() {
...
function renderTodoItem(t, depth) {
  ...
  let collapseBtn = '';
  if (depth < 2 && kids) {
    collapseBtn = `<button class="td-collapse${collapsed ? ' closed' : ''}" data-action="toggleCollapse" data-id="${t.id}"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"/></svg></button>`;
  } else if (depth === 0) {
    collapseBtn = '<div class="td-collapse-spacer"></div>';
  }

  // Add sub-task button (only for depth 0 and 1)
  const addBtn = depth < 2 ? `<button class="td-act-btn add" data-action="showAddChild" data-id="${t.id}" title="Add sub-task"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></button>` : '';

  let html = `<div class="td-item" data-id="${t.id}">
    ${collapseBtn}
    <button class="td-check${doneClass}" data-action="toggleTodoDone" data-id="${t.id}" data-done="${!t.done}">${t.done ? checkSvg : ''}</button>
    ...
    <div class="td-actions">
      ${addBtn}
      <button class="td-act-btn del" data-action="deleteTodo" data-id="${t.id}" title="Delete"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
    </div>
  </div>`;
...
export function addTodoFromInput() {
...
document.addEventListener('click', function(e) {
  ...
  if (action === 'openSopDoc') {
    openSopDoc(el.dataset.docId);
  } else if (action === 'toggleCollapse') {
    e.stopPropagation();
    toggleCollapse(el.dataset.id);
  } else if (action === 'toggleTodoDone') {
    e.stopPropagation();
    toggleTodoDone(el.dataset.id, el.dataset.done === 'true');
  } else if (action === 'showAddChild') {
    e.stopPropagation();
    showAddChild(el.dataset.id);
  } else if (action === 'deleteTodo') {
    e.stopPropagation();
    deleteTodo(el.dataset.id);
  } else if (action === 'showSvSearch') {
    showSvSearch();
  } else if (action === 'switchSvTab') {
    switchSvTab(el.dataset.tabId);
  } else if (action === 'closeSvTab') {
    e.stopPropagation();
    closeSvTab(el.dataset.tabId);
  } else if (action === 'toggleSvMaximize') {
    toggleSvMaximize();
  } else if (action === 'closeSopViewer') {
    closeSopViewer();
  } else if (action === 'fillSvChat') {
    fillSvChat(el.dataset.question);
  } else if (action === 'askSvDoc') {
    askSvDoc();
  } else if (action === 'runSvSearch') {
    runSvSearch();
  } else if (action === 'openSvSearchResult') {
    openSvSearchResult(el.dataset.docId);
  } else if (action === 'discoverSops') {
    discoverSops();
  } else if (action === 'searchSops') {
    searchSops();
  }
});

// Expose functions called from inline onkeydown/onblur handlers in dynamic HTML
window.addTodo = addTodo;
window.editTodoTitle = editTodoTitle;
window.runSvSearch = runSvSearch;
window.askSvDoc = askSvDoc;
