...
import { searchSops } from './todos.js';
// Hub was removed — dismissVentHub is a no-op
function dismissVentHub() {}

// i18n helper
function t(k){ return (window.VentI18n && VentI18n.t(k)) || k; }

// ── Panel registry: functions from other modules registered by main.js ──
let panelRegistry = {};

export function registerPanels(map) {
  Object.assign(panelRegistry, map);
}

// ── Build/scroll helpers registered from query-engine ──
let _buildAnswerHtml = null;
let _scrollToBottom = null;

export function registerHelpers({ buildAnswerHtml, scrollToBottom }) {
  _buildAnswerHtml = buildAnswerHtml;
...
var tourSteps = [
  ...
  // 5 — Ask a Question (todo sidebar removed)
  // 6 — Ask a Question
...
window.DEMO_FALLBACK_RESPONSE = DEMO_FALLBACK_RESPONSE;
  ...
  { target:'#qInput', titleKey:'demo.d2.title', descKey:'demo.d2.desc', voiceKey:'demo.d2.voice', pos:'top',
    setup: function(){
      ...
      setTimeout(function(){
        ...
        if(_scrollToBottom) _scrollToBottom();
        ...
        setTimeout(function(){
          ...
          if(think && _buildAnswerHtml){
            think.outerHTML = _buildAnswerHtml(DEMO_FALLBACK_RESPONSE);
          }
          if(_scrollToBottom) _scrollToBottom();
        }, 3200);
      }, typeLen);
    ...
    teardown: function(){
      ...
      if(!document.querySelector('.answer-card')){
        ...
        if(inner && _buildAnswerHtml){
          ...
          turn.innerHTML =
            '<div class="turn-q"><div class="turn-q-bubble">' + DEMO_QUESTION + '</div></div>' +
            _buildAnswerHtml(DEMO_FALLBACK_RESPONSE);
          inner.appendChild(turn);
        }
      }
      if(_scrollToBottom) _scrollToBottom();
...
function closeOpenPanel(){
  if(openPanelClose){
    try {
      if(panelRegistry[openPanelClose]) panelRegistry[openPanelClose]();
    } catch(e){}
    openPanelClose = null;
  }
}
  ...
  .then(function(data){
    ...
    if(answer){
      ...
      if(ans){
        ...
        ans.innerHTML = '<div class="tour-ask-q">\u201c' + escHtml(question) + '\u201d</div>' + answer;
      }
      speakStep(answer);
...
function renderStep(){
  ...
  if(!el){
    ...
    else { if(step.teardown) try{ step.teardown(); }catch(e){} tourIdx++; if(tourIdx >= tourSteps.length) endTour(); else renderStep(); }
    return;
  }
  ...
  var btns = '';
  if(canGoBack) btns += '<button class="tour-back" data-action="prevTourStep">' + t('tour.back') + '</button>';
  if(hasSub) btns += '<button class="tour-explore" data-action="exploreTourStep">' + t('tour.explore') + '</button>';
  var nextLabel = (isLastMain && !isSubStep && !hasSub) || (isLastMain && isLastSub) ? t('tour.done') : t('tour.next');
  btns += '<button class="tour-next" data-action="nextTourStep">' + nextLabel + '</button>';
  ...
  var askBtn = isVoiceTour
    ? '<button class="tour-ask-btn" data-action="askCharlieTour" title="' + t('tour.askHint') + '">' +
  ...
  var voiceRow = isVoiceTour
    ? '<div class="tour-voice-row">' +
        ...
        '<button class="tour-voice-mute'+(voiceMuted?' muted':'')+'" data-action="toggleTourMute">'+muteIcon+'</button>' +
        askBtn +
      '</div>'
    : '';
  ...
  card.innerHTML =
    ...
    '<div class="tour-card-actions">' +
      '<button class="tour-skip" data-action="endTour">' + t('tour.skip') + '</button>' +
...
export function exploreTourStep(){
  var mainStep = tourSteps[tourIdx];
  if(!mainStep.sub || !mainStep.sub.length) return;
  if(mainStep.open){
    try {
      if(panelRegistry[mainStep.open]) panelRegistry[mainStep.open]();
    } catch(e){}
  }
...
export function startTour(){ start(false); }
export function startVoiceTour(){ start(true); }

// Delegated click handler for tour data-action elements
document.addEventListener('click', function(e) {
  var el = e.target.closest('[data-action]');
  if (!el) return;
  var action = el.dataset.action;

  if (action === 'prevTourStep') {
    prevTourStep();
  } else if (action === 'nextTourStep') {
    nextTourStep();
  } else if (action === 'exploreTourStep') {
    exploreTourStep();
  } else if (action === 'endTour') {
    endTour();
  } else if (action === 'toggleTourMute') {
    toggleTourMute();
  } else if (action === 'askCharlieTour') {
    askCharlie();
  }
});
...
if(new URLSearchParams(window.location.search).get('demo') === '1'){
  setTimeout(function(){ startDemoTour(); }, 800);
}
