...
// ════════════════════════════════════════════════════════
// CANVAS — reusable particle system
// ════════════════════════════════════════════════════════
function initParticleCanvas(canvasId, { count = 50, speed = 0.25, maxSize = 1.3, lineRange = 130, lineAlpha = 0.07, delay = 300 } = {}) {
  ...
  for (let i = 0; i < count; i++) {
    particles.push({
      ...
      size: Math.random() * maxSize + 0.3, opacity: Math.random() * 0.25 + 0.04,
  ...
  function draw() {
    ...
    for (let i = 0; i < particles.length; i++) {
      for (let j = i + 1; j < particles.length; j++) {
        ...
        if (dist < lineRange) {
          ctx.strokeStyle = `rgba(0,122,204,${(1 - dist / lineRange) * lineAlpha})`;
          ctx.lineWidth = 0.5;
          ctx.beginPath();
  ...
  draw();
  setTimeout(() => canvas.classList.add('active'), delay);
}

function initCanvas() {
  initParticleCanvas('vizCanvas', { count: 60, speed: 0.3, maxSize: 1.5, lineRange: 150, lineAlpha: 0.08, delay: 500 });
}

function initLoginCanvas() {
  initParticleCanvas('loginCanvas', { count: 40, speed: 0.2, maxSize: 1.2, lineRange: 120, lineAlpha: 0.06, delay: 300 });
}
